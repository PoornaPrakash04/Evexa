...
/**
 * Resolve a venue_id from the request.
 * Accepts ?venue_id (preferred) or ?venue_name (fallback).
 * Calls cb(err, venue_id).
 */
function resolveVenueId(query, cb) {
  if (query.venue_id) {
    const id = Number(query.venue_id);
...
router.get("/", auth(["ORGANIZER", "STUDENT", "FACULTY"]), (req, res) => {
  db.query(
    "SELECT id, name, capacity, location FROM venues",
    (err, results) => {
...
router.post("/bookings", auth(["ORGANIZER"]), upload.single("support_doc"), (req, res) => {
  const { venue_id, event_id, date, slot_start, slot_end, purpose } = req.body;
 
  if (!venue_id || !date || !slot_start || !slot_end) {
    return res.status(400).json({ message: "venue_id, date, slot_start, and slot_end are required." });
  }
 
  const vId = Number(venue_id);
  if (!vId) return res.status(400).json({ message: "Invalid venue_id." });
  ...
  db.getConnection((connErr, connection) => {
    ...
    connection.query("SELECT id FROM venues WHERE id = ?", [vId], (err, rows) => {
      if (err || !rows.length) {
        connection.release();
        return res.status(err ? 500 : 400).json({ message: err ? err.message : "Venue not found." });
      }
...
router.put("/bookings/:id/status", auth(["HALL_COORDINATOR"]), (req, res) => {
  ...
  if (!["hall_approved", "rejected"].includes(status)) {
    return res.status(400).json({ message: "Invalid status. Use 'hall_approved' or 'rejected'." });
  }
  ...
  db.query(
    ...
    (err, results) => {
      ...
      if (booking.status !== "faculty_approved") {
        return res.status(400).json({ message: "Booking must be faculty_approved first." });
      }
